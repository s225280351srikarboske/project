...
function renderProperties(properties) {
  ...
  tbody.innerHTML = properties
    .map((p) => {
      const img = Array.isArray(p.images) && p.images.length ? p.images[0] : 'https://via.placeholder.com/60x40';
      ...
      return `
        <tr>
          <td><img src="${escapeHtml(img)}" alt="Property" class="property-image"></td>
...
async function handlePropertySubmit(e) {
  ...
  try {
    if (propertyId) {
      await api(`/api/properties/${propertyId}`, 'PUT', payload);
    } else {
